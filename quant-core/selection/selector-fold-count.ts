...
export function evaluateGate0ForExpectedFoldCountV1(
  candidate: InnerCandidateEvidenceV1,
  expectedFoldCount: number,
): Gate0ResultV1 {
  if (!Number.isInteger(expectedFoldCount) || expectedFoldCount <= 0) throw new Error('INVALID_EXPECTED_FOLD_COUNT');
  const reasons: string[] = [];
  if (!finiteMetricSet(candidate)) reasons.push('NONFINITE_METRICS');
  if (candidate.folds.length !== expectedFoldCount) reasons.push('INNER_FOLD_COUNT_MISMATCH');
...
export function selectLabelCandidateForExpectedFoldCountV1(
  candidates: readonly InnerCandidateEvidenceV1[],
  expectedFoldCount: number,
): LabelSelectionEvidenceV1 {
  if (!Number.isInteger(expectedFoldCount) || expectedFoldCount <= 0) throw new Error('INVALID_EXPECTED_FOLD_COUNT');
  const trace = candidates.map((candidate) => {
    const gate0 = evaluateGate0ForExpectedFoldCountV1(candidate, expectedFoldCount);
